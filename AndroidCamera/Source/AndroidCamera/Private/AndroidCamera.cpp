...
FAndroidCameraModule& FAndroidCameraModule::Get() {
  return *reinterpret_cast<FAndroidCameraModule*>(FModuleManager::Get().
    GetModule("AndroidCamera"));
}

void FAndroidCameraModule::RegisterComponent(UAndroidCameraComponent& Component,
                                             int DesiredWidth,
                                             int DesiredHeight,
                                             int DesiredFPS) {
...
void FAndroidCameraModule::ActivateComponent(int CameraId, int PreviewWidth,
                                             int PreviewHeight,
                                             int CameraRotation) {
  if (PendingComponents.size()) {
    UAndroidCameraComponent* Component = PendingComponents.front();
    PendingComponents.pop();
    Component->ActivateComponent(CameraId, PreviewWidth, PreviewHeight,
                                 CameraRotation);
    IdToComponents.insert({CameraId, Component});
  } else {
    UE_LOG(LogCamera, Display,
           TEXT("Something went wrong. Cannot find pending component for id %d"
           ), CameraId);
  }
}
...
UAndroidCameraComponent* FAndroidCameraModule::GetComponent(int CameraId) {
  ...
  if (it == IdToComponents.end()) {
    UE_LOG(LogCamera, Display, TEXT("Cannot find component with id %d"),
           CameraId);
    return nullptr;
  } else {
    return it->second;
...
bool FAndroidCameraModule::OnImageAvailable(int CameraId, unsigned char* Y,
                                            unsigned char* U, unsigned char* V,
                                            int YRowStride, int UVRowStride,
                                            int UVPixelStride, int YLength,
                                            int ULength, int VLength) {
  TRACE_ANDROIDCAMERA_ON_FRAME(CameraId);
  if (UAndroidCameraComponent* Component = FAndroidCameraModule::Get().
      GetComponent(CameraId)) {
    Component->OnImageAvailable(Y, U, V, YRowStride, UVRowStride, UVPixelStride,
