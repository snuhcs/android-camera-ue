...
bool AndroidThunkCpp_LoadVideo(FString VideoPath)
{
	...
	{
		static jmethodID Method = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "AndroidThunkJava_LoadVideo", "(Ljava/lang/String;)Z", false);
		jstring path = Env->NewStringUTF(TCHAR_TO_ANSI(*VideoPath));
		return FJavaWrapper::CallBooleanMethod(Env, FJavaWrapper::GameActivityThis, Method, path);
	}
	return false;
}
...
bool AndroidThunkCpp_GetNFrames(int N, int W, int H, int* outputBuffer)
{
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		static jmethodID Method = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "AndroidThunkJava_GetNFrames", "(ILjava/nio/ByteBuffer;)Z", false);
		jobject jArr = Env->NewDirectByteBuffer(outputBuffer, N * W * H * 4);
		bool ret = FJavaWrapper::CallBooleanMethod(Env, FJavaWrapper::GameActivityThis, Method, N, jArr);
		return ret;
	}
	return false;
}
...
bool AndroidThunkCpp_CloseVideo()
{
	if(JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		static jmethodID Method = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "AndroidThunkJava_CloseVideo", "()Z", false);
