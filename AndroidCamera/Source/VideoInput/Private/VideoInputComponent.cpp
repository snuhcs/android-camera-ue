...
{
	Super::BeginPlay();
	UE_LOG(LogVideo, Display, TEXT("UVideoInputComponent BeginPlay! Don't forget to OpenVideo"));
}
...
void UVideoInputComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	UE_LOG(LogVideo, Display, TEXT("UVideoInputComponent EndPlay! Killing FetchEngine..."));
	
	KillEngine();
}

void UVideoInputComponent::Initialize(FString path, int w, int h, int totalFrameCnt, int blockFrameCnt, int buffFrameCnt, float remainingPercent, int64 frameDuration)
{
	UE_LOG(LogVideo, Display, TEXT("TotalFrameCnt must be divisble by BufferFrameCnt && BufferFrameCnt must be divisible by BlockFrameCnt."))
	
	VideoFilePath = path;
	if(!FVideoInputModule::Get().CallJava_LoadVideo(VideoFilePath))
	{
		UE_LOG(LogVideo, Display, TEXT("LoadVideo failed! Please check video path (%s)"), *VideoFilePath);
		return;
	}
	RequireFetch = true; // No need to access with mutex because StartVideo writes first before any other function
...
void UVideoInputComponent::FetchLoop()
{
	while(true)
	{
		...
		UE_LOG(LogVideo, Display, TEXT("Fetching frames %d ~ %d"), FetchHead, FetchHead + BatchFrameCnt);
		if(FVideoInputModule::Get().CallJava_GetNFrames(BatchFrameCnt, W, H, &Buffer[FrameToInt(FetchHead)]))
		{
...
void UVideoInputComponent::ConsumeLoop()
{
	while(true)
	{
		...
		while(cnt > 0)
		{
			...
			UE_LOG(LogVideo, Display, TEXT("OnImageAvailableDelegate called on frame %d"), ConsumeHead);
			BroadcastImageAvailability(ConsumeHead);
			
			ConsumeHead++;
			if(!signalSent && cnt <= BatchFrameCnt * RemainingPercent) // If frames left to read are less than RemainingPercent
			{
				// Notify Fetcher
				std::unique_lock<std::mutex> FetchLock(FetchSignalMtx);
				RequireFetch = true;
